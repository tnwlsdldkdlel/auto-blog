...
for (const label of ['전체공개', '비공개', '댓글허용', '공감허용']) {
  ...
  const info = await el.evaluate((node, fnStr) => {
    ...
    const cand =
      node.closest('[role="checkbox"],[role="switch"],label,button,li') ??
      node.parentElement ??
      node;
    return {
      tag: node.tagName,
...
const before = await commentEl.evaluate(
  (n, f) => new Function('node', 'return (' + f + ')(node)')(n),
  readStateFn.toString(),
);
await commentEl.click();
await page.waitForTimeout(400);
const after = await commentEl.evaluate(
  (n, f) => new Function('node', 'return (' + f + ')(node)')(n),
  readStateFn.toString(),
);
console.log(
  `  before=${before} → after=${after} ${before !== after ? '✓ 상태 변화 감지됨(토글+상태읽기 OK)' : '⚠ 변화 없음(상태읽기 실패 가능)'}`,
);
// 원상복구
